...
int SDL2InputBackend::GetMouseY()
{
	...
	int relativeY = (mouseY - rect.y) * (windowHeight / rect.h);
	return static_cast<int>(relativeY);
}

void SDL2InputBackend::SetMouseX(int x)
{
	...
	int windowX = rect.x + (x * rect.w / renderTargetWidth);
	int windowY;
...
void SDL2InputBackend::SetMouseY(int y)
{
	if (!backend->GetGraphics()) return;
	SDL_Window* window = backend->GetGraphics()->GetSDLWindow();
	if (!window) return;
	...
	int windowX;
	int windowY = rect.y + (y * rect.h / renderTargetHeight);
...
uint32_t SDL2InputBackend::GetMouseState()
{
	...
	return touchDown ? 1 << 0 : 0;
#else
	return SDL_GetMouseState(nullptr, nullptr);
#endif
}

void SDL2InputBackend::HideMouseCursor()
{
	SDL_ShowCursor(SDL_DISABLE);
}

void SDL2InputBackend::ShowMouseCursor()
{
...
uint8_t SDL2InputBackend::GetGamepadButtonState(int index)
{
	...
	if (SDL_GameControllerGetButton(gamepads.at(index), SDL_CONTROLLER_BUTTON_A)) state |= 1 << 4;
	if (SDL_GameControllerGetButton(gamepads.at(index), SDL_CONTROLLER_BUTTON_B)) state |= 1 << 5;
	if (SDL_GameControllerGetButton(gamepads.at(index), SDL_CONTROLLER_BUTTON_X)) state |= 1 << 6;
	if (SDL_GameControllerGetButton(gamepads.at(index), SDL_CONTROLLER_BUTTON_Y)) state |= 1 << 7;
